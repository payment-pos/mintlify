...
| `validate_date` | datetime \| null | Transferin onaylandığı (approve) zaman |
...
İstek reddi veya iş kuralı ihlali durumunda Payven [RFC 9457 problem+json](/documentation/concepts/errors) zarfı döner. Tipik hatalar:
...
`failed` durumda olan bir transferin sebebi sonradan `GET /api/v1/transfers/{id}` ile sorgulanabilir; banka detayları yanıt gövdesindeki `error_code` ve `provider_error_code` alanlarında taşınır.
