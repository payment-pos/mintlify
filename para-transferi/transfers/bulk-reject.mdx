...
<Note>
**Reject ≠ İptal.** Bir transfer **bankaya gönderildikten** sonra reject edilemez (`pending` durumunda değildir). Bankaya gitmiş bir transferin geri alınması için banka tarafı iade akışı (sonradan `para-transferi.transfer.refunded` olabilir) gerekir.
</Note>
...
Reddedilen her transfer için `para-transferi.transfer.rejected` webhook olayı yayınlanır (`status: "rejected"`, red gerekçesi `message` alanında). Para hiç hareket etmediği için bakiye etkisi yoktur. Ayrıntı: [Olay Kataloğu](/para-transferi/webhooks/events).
