...
flowchart LR
    ...
    D --> E[para-transferi.transfer.completed<br/>veya .failed webhook]
```
...
Tetikleme anında kaynak hesapta yeterli bakiye yoksa transfer **`failed`** statüsünde oluşur (`error_code: "insufficient_balance"`). Tekrarlayan kayıt **devam eder** — sonraki periyodda yeniden denenir. Operasyonel olarak `para-transferi.transfer.failed` webhook'una abone olmanız önerilir.
...
Tekrarlayan kayıt her günlük çekilimin sonucu için kendi olaylarını yayınlar:

| Olay | Tetikleyici |
|---|---|
| `para-transferi.recurring_execution.succeeded` | Günlük çekilim bankaya başarıyla iletildi (`transfer_request_id` üretilen transferi taşır) |
| `para-transferi.recurring_execution.failed` | Günlük çekilim başarısız oldu; plan hâlâ aktif (`error_message`, `consecutive_failure_count`) |
| `para-transferi.recurring_transfer.suspended` | Plan ardışık hatalar nedeniyle otomatik askıya alındı — `resume` ile manuel devam ettirmeniz gerekir |

Ayrıca ürettiği her transfer terminal sonuca ulaştığında standart `para-transferi.transfer.completed` / `.delivered` / `.failed` olayları yayınlanır. Olaylar `recurring_transfer_id` ve `execution_id` taşır; üretilen transfer `transfer_request_id` alanındadır. Payload ayrıntısı: [Olay Kataloğu](/para-transferi/webhooks/events).
