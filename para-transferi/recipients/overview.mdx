...
|---|---|---|---|
| `holder_name` | string | ✅ | Alıcının adı (max 100 karakter) |
| `tax_id` | string | ✅ | TC kimlik no (gerçek kişi) veya vergi numarası (tüzel kişi) |
| `tax_id_type` | enum | ✅ | `tckn` (gerçek kişi) veya `vkn` (tüzel kişi) |
| `email` | string | ❌ | Bilgilendirme e-postası gönderilecek adres (raporlama için) |
| `phone` | string | ❌ | İletişim numarası |
| `label` | string | ❌ | Operatör için etiket (örn. "Maaş listesi 2026 Mayıs") |
...
Standart sayfalı liste zarfı döner.
