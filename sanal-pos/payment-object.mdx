...
| `is_success` | bool | İşlemin başarı/başarısızlığı. **HTTP 200 olsa bile bu alan `false` olabilir** (örn. banka reddi). |
| `message` | string \| null | Banka veya Payven tarafından dönen açıklama (insan-okur, Türkçe) |
| `error_code` | string \| null | Hata kodu — başarısız işlemlerde dolar (snake_case). Örn. `bank_declined`, `insufficient_funds` |
| `provider_error_code` | string \| null | Bankanın orijinal yanıt kodu (Visa/MC/banka spec'i, örn. `51` = yetersiz bakiye) |
...
`GET /api/v1/payments/{transaction_id}` ile sorgulamada `PaymentOperationResult` üzerine **ek alanlar** koyar:
...
İşlem reddi / iş kuralı ihlali durumunda Payven [RFC 9457 problem+json](/documentation/concepts/errors) zarfı döner. Banka tarafı reddinde HTTP `200` ile birlikte `is_success: false` ve `error_code: "bank_declined"` da gelebilir — başarı kararını sadece HTTP koduna değil, gövdeye de bakarak verin:
...
```json
{
  ...
  "extra_properties":    { "processed_at": "2026-05-03T12:34:58.123+00:00" }
}
```

`provider_error_code` banka tarafından dönen orijinal koddur. Tam liste: [Banka Yanıt Kodları](/sanal-pos/errors/bank-codes).
