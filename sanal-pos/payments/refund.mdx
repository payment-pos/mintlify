...
|---|---|---|---|
| `amount` | int (kuruş) | ❌ | İade tutarı. Boş bırakılırsa **kalan tüm tutar** iade edilir. |
| `currency` | string | ❌ | Para birimi. Belirtilmezse orijinal işlemin para birimi kullanılır. |
| `reason` | string | ❌ | Serbest metin iade notu (max 500 karakter). Raporlamada görünür. |
| `reason_code` | enum | ❌ | Yapılandırılmış iade sebebi: `customer_request`, `duplicate`, `fraudulent`, `product_not_received`, `product_unsatisfactory`, `subscription_canceled`, `other` |
