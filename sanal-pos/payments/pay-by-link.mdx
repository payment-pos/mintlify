...
sequenceDiagram
    ...
    Payven-)App: webhook (sanalpos.payment.completed)
```
...
| Olay | Açıklama |
|---|---|
| `sanalpos.payment.completed` | Müşteri linkten ödemeyi başarıyla yaptı |
| `sanalpos.payment.failed` | Müşteri ödeme yaptı ama banka reddetti |
| `sanalpos.three_ds.completed` / `sanalpos.three_ds.failed` | 3DS akışı tetiklendiğinde |
...
<AccordionGroup>
  <Accordion title="Çağrı merkezi">
    ...
    4. Webhook ile `sanalpos.payment.completed` geldiğinde sipariş onaylanır.
  </Accordion>
