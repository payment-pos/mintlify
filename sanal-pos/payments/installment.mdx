...
2. **Kuruluşunuzun banka anlaşması** (her banka kendi taksit kampanyalarını tanımlar)
...
<Tabs>
  ...
  <Tab title="Direct API">
    ...
    ```bash
    curl "https://identity.payven.com.tr/api/v1/lookups/bank-bins?bin_prefix=454671&page_size=1" \
      -H "Authorization: Bearer $PAYVEN_TOKEN"
    ```

    Yanıt; bankayı, kart programını ve kart tipini (`credit/debit/prepaid`) verir. Taksit kampanyaları ise konsoldan tanımlanan **kuruluş anlaşma tablosundan** çekilir — yakında public bir `/installments` endpoint'i ile dinamik sorgulanabilir hale gelecektir.
...
<Check>**Komisyon kart programına bağlı.** Her banka kendi taksit kampanyasını tanımlar — kuruluş anlaşmanıza yansır.</Check>
