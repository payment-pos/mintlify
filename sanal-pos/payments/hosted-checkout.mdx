...
sequenceDiagram
    ...
    Payven-)App: webhook (asenkron — sanalpos.payment.completed)
```
...
```json
{
  ...
  "checkout_url":      "https://vpos.payven.com.tr/s/Ab3kX9qZmn",
  "checkout_page_url": "https://vpos.payven.com.tr/checkout/classic/?session=8e3f5c129a7b4c8dbc4e2c963f66afa6",
...
| `checkout_url` | Müşteriyi yönlendireceğiniz **kısa** URL. SMS/e-posta/QR ile paylaşmaya uygundur; açıldığında ödeme sayfasına 302 ile yönlenir. Kısa link üretilemezse `checkout_page_url` ile aynı değeri taşır. |
| `checkout_page_url` | Ödeme sayfasının doğrudan (uzun) adresi. Yalnız kısa link istemeyen entegrasyonlar için. |
| `expires_at` | Session geçerlilik süresi (oluşturulduktan **30 dakika** sonra) |

<Note>
Kısa link yalnızca müşteriye gönderdiğiniz adres için geçerlidir. 3D Secure callback'i,
`success_url`/`fail_url` yönlendirmeleri ve webhook'lar değişmez; bunlar işlem kimliği
(`transaction_id`) ile çalışmaya devam eder.
</Note>
...
| Olay | Açıklama |
|---|---|
| `sanalpos.payment.completed` | Müşteri başarıyla ödeme tamamladı |
| `sanalpos.payment.failed` | Müşteri ödeme yapmaya çalıştı, başarısız |
| `sanalpos.three_ds.completed` / `sanalpos.three_ds.failed` | 3DS akışlı modlarda ek olarak yayınlanır |
