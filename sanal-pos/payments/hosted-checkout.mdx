...
<Check>**Kuruluş varsayılan 3DS modu** — kuruluşunuzun policy'sinde tanımlı 3DS akışı (klasik 3D Secure, 3D Pay veya Hosted) otomatik seçilir</Check>
...
sequenceDiagram
    ...
    Payven-)App: webhook (asenkron — payment.succeeded)
```
...
| `payment_mode` | enum | ❌ | `non_3d` / `three_d_secure` / `three_d_pay` / `three_d_pay_hosting`. Boş → kuruluşunuzun `preferred_three_d_flow` ayarından türetilir. |
...
<Note>
**Kuruluş politikası uyumu:** `payment_mode: "non_3d"` istenirse ancak kuruluşunuzun
politikasında `allow_non_3d` kapalıysa istek `400 Bad Request` ile reddedilir. Production
kurulumlarda Non-3D fraud riski nedeniyle varsayılan olarak kapalıdır.
</Note>
...
- Konsoldan kuruluş ayarlarını kullanın (logo, ana renk)
...
| HTTP | `code` | Anlam |
|---|---|---|
| `400` | `non_3d_not_allowed` | Kuruluş politikasında Non-3D kapalı |
