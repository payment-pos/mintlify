...
|---|---|---|---|
| `external_id`, `amount`, `installment`, `card`, `buyer`, `billing_address`, `shipping_address`, `basket_items` | — | — | [Non-3D](/sanal-pos/payments/non-3d) ile aynı |
| `operation_type` | enum | ❌ | `sale` (varsayılan) veya `pre_auth` |
| `callback_url` | url | ✅ | Bankanın 3DS sonrası form-post yapacağı endpoint. **Sunucu tarafında** Payven'in `/3d/callback` endpoint'ini kullanmanız önerilir; bu alanı yalnızca özel akışlarda kendi endpoint'inizle override edin. |
| `return_url` | url | ✅ | Müşterinin son olarak yönlendirileceği URL. Genellikle frontend sayfası. |
| `cancel_url` | url | ❌ | Müşteri 3DS sayfasını iptal ederse yönlendirileceği URL |
...
```json
{
  ...
  "status":              "pending_3ds",
  "is_success":          true,
  "message":             "3DS yönlendirme başlatıldı",
  "error_code":          null,
...
`extra_properties.redirect_url`'e müşteriyi **HTTP 302** ile yönlendirin (veya tarayıcıda `window.location.href`).
