...
<Note>
İşlem durumunun wire değeri **`canceled`** (önceki ad "voided" değiştirildi, eski isimlendirme tüm enum genelinde temizlendi). Asenkron void senaryosunda geçici olarak `canceled_processing` durumu görülebilir; final durum webhook (`sanalpos.void.completed`) ile gelir.
</Note>
...
| Olay | Açıklama |
|---|---|
| `sanalpos.void.completed` | İptal başarıyla tamamlandı (`data.status: "canceled"`) |
| `sanalpos.void.failed` | İptal banka tarafından reddedildi |
