...
| `amount.currency` | enum | ✅ | Şu an yalnız `"TRY"` kabul edilir. Yabancı kart için DCC akışında çevrim yanıtta döner — bkz. [Tutarlar](/documentation/concepts/amounts) |
...
Banka işlemi reddederse yanıt **`422 Unprocessable Entity` + `application/problem+json`** olarak döner:
...
Yanıt başarılı mı diye HTTP durum kodunu kontrol edin. Banka tarafı detayını (orijinal banka kodu, auth_code vb.) görmek için işlem sonradan `GET /api/v1/payments/{id}` ile sorgulanabilir — failed ödemenin sorgusu `200 OK` döner ve `error_code` / `provider_error_code` alanları yanıt gövdesinde data olarak taşınır.
