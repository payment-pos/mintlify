...
Sırrı yeniledikten sonraki geçiş süresinde başlık **virgülle ayrılmış iki imza** taşır
(`sha256=<yeni>,sha256=<eski>`). Başlığı virgülden bölün; imzalardan **herhangi biri** sizin
hesapladığınızla eşleşiyorsa isteği kabul edin.

İmza şu formülle üretilir:

```
imzalanacak_string = "<X-Payven-Timestamp>" + "." + "<request_body_raw_string'i>"
hmac               = HMAC_SHA256(webhook_ucu.secret, imzalanacak_string)
...
3. Başlığı virgülden bölüp her imzayı sabit zamanlı (constant-time) karşılaştırma ile doğrulayın
...
app.post("/webhooks/payven", (req, res) => {
  ...
  // 3. Sabit zamanlı karşılaştır — başlık virgülle birden fazla imza taşıyabilir
...
async def webhook(request: Request):
    ...
    # 3. Sabit zamanlı karşılaştır — başlık virgülle birden fazla imza taşıyabilir
    if not any(hmac.compare_digest(sig.strip(), expected) for sig in signature.split(",")):
...
public class WebhookController : ControllerBase
{
    ...
    public async Task<IActionResult> Receive()
    {
        ...
        // 4. Sabit zamanlı karşılaştır — başlık virgülle birden fazla imza taşıyabilir
        var expectedBytes = Encoding.UTF8.GetBytes(expected);
        var valid = signature
            .Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries)
            .Any(sig => CryptographicOperations.FixedTimeEquals(Encoding.UTF8.GetBytes(sig), expectedBytes));
        if (!valid)
        {
...
foreach (explode(",", $signature) as $sig) {
    if (hash_equals($expected, trim($sig))) { $valid = true; break; }
}
if (!$valid) {
    http_response_code(401);
...
Webhook ucunun sırrını İşyeri API'si üzerinden yenileyin:

```bash
curl -X POST https://merchant.payven.com.tr/api/v1/webhook-endpoints/8e3f5c12-.../rotate-secret \
  -H "Authorization: Bearer $PAYVEN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{ "expire_current_in_hours": 24 }'
```

Yanıt yeni sırrı (`secret`) **bir kez** döner. Eski sır `expire_current_in_hours` boyunca
(0–168, varsayılan 24) geçerli kalır: bu sürede Payven her isteği **iki sırla da imzalar** ve
başlık `sha256=<yeni>,sha256=<eski>` biçiminde gelir. Yukarıdaki örnekler bu başlığı zaten
virgülden böldüğü için yeni sırrı dağıtırken teslimat kesilmez. Sır sızdıysa `0` verin — eski sır
hemen geçersiz olur.
...
| Sır yenilemeden sonra imza tutmuyor | Başlığı virgülden bölmüyorsunuz — geçiş süresinde iki imza gelir, birini kabul edin |
