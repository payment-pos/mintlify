...
JWT içinde `merchant_id` claim'i taşınır — tek-merchant kurulumlarda ek bir header göndermenize gerek yoktur. Sanal POS, claim'den okuyup işlemi doğru merchant adına kayıt eder.

**Multi-merchant senaryolar** (bir kuruluş altında birden çok merchant'a işlem alıyorsanız) için override header'ları:
...
|---|---|---|---|
| `X-Merchant-Id` | UUID | Payven internal kimlik | Konsoldan kopyalanır (Merchant.id) |
