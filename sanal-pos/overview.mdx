...
|---|---|---|
| Ödeme oluşturma | `POST /payments`, `POST /payments/3d/init`, `POST /checkout/sessions` | JWT Bearer |
| Ödeme aksiyonları | `POST /payments/{id}/refund`, `/void`, `/capture` | JWT Bearer |
| Sorgulama | `GET /transactions`, `GET /payments/{id}` | JWT Bearer |
| Webhook yönetimi | `POST /webhooks` | JWT Bearer |
| Mutabakat | `POST /reconciliations/start` | JWT Bearer |
| Backoffice | `GET /connectors`, `/connector-configurations` | JWT Bearer |
