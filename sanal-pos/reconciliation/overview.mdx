...
Mutabakat (reconciliation), bir dönemde yapılan tüm ödeme/iade hareketlerinin **banka kayıtlarıyla eşleştirilmesi** sürecidir. Tutarsızlıkları (eksik kayıt, tutar farkı, durum farkı) tespit edip operatörün karar vermesini bekler; nihai onayla gelir muhasebenize netlik kazandırır.
...
|---|---|---|---|
| `scope` | enum | ✅ | `connector` veya `merchant` |
...
| `period` | enum | ✅ | `daily`, `monthly`, `yearly`, `custom` |
| `period_start` | datetime | ✅ | Dönem başlangıcı (UTC) |
| `period_end` | datetime | period=custom | Dönem sonu — `custom` için zorunlu |
| `bank_transactions[]` | array | ❌ | Banka satır listesi. Boş bırakılırsa sadece DB tarafı ile çalışılır (banka veriden bağımsız Payven self-reconciliation). |
