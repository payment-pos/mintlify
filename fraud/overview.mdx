...
Her ödeme isteğinde Sanal POS, fraud servisinden iki çağrı yapar:
...
sequenceDiagram
    ...
    POS->>Fraud: POST /internal/fraud/evaluate
    alt Karar: allow
        Fraud-->>POS: { decision: "allow", score: 12 }
        POS->>Bank: Otorizasyon
        Bank-->>POS: Onay
        POS->>Fraud: POST /internal/fraud/finalize
        POS-->>App: completed
    else Karar: block
        ...
        Note over POS: İşlem holding'de — operatör onayı bekleniyor
    end
```
...
| Kategori | Açıklama |
|---|---|
| `/internal/fraud/*` | Sanal POS'un dahili kullanımı — `evaluate`, `finalize` |
| `/fraud/rules` | Tenant kuralları CRUD |
| `/tenant/fraud/{rules,blacklist,alerts,logs,merchant-policies}` | Tenant bazlı yönetim |
| `/platform/fraud/*` | Platform-level (multi-tenant) kara liste, alert havuzu |
