...
| `correlationId` | — | İstek zinciri kimliği. Yanıt header'ı `X-Correlation-Id` ile aynı. Destek talebinde paylaşın. |
...
Her hata yanıtında dönen **`correlationId`** değerini paylaşın — log zincirini saniyeler içinde bulabiliriz. Yanıt header'ındaki `X-Correlation-Id` ile gövdedeki `correlationId` aynı değerdir.

```text
Konu:   Refund 422 — refund_exceeds_original
Detay:  correlationId: 9f1c8e76-2a3b-4f12-9c8d-12cb24a8a8a8
        instance:      /api/v1/payments/8e3f5c12-.../refund
