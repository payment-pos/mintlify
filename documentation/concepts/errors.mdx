...
<Note>
**HTTP `200` her zaman ödeme başarılı demek değildir.** Banka tarafında reddedilen bir
işlem `200` ile dönüp gövdesinde `is_success: false`, `error_code: "bank_declined"`,
`provider_error_code: "51"` (yetersiz bakiye) taşıyabilir. Asıl başarı kararını
yanıttaki `is_success` ve `status` alanlarına bakarak verin.
</Note>
...
| `invalid_token` | 401 | JWT geçersiz veya tampere edilmiş |
...
| `tenant_not_found` | 404 | Auth endpoint'inde `{slug}` geçersiz |
| `tenant_suspended` | 403 | Kuruluş askıya alınmış |
...
| `validation_failed` | 422 | Validasyon kuralı ihlali — hatalı alanların listesi `errors[]` içinde döner |
