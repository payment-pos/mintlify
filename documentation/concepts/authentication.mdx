...
<AccordionGroup>
  <Accordion title="Tek bir secret ile doğrudan API çağıramaz mıyım?">
    Hayır — Payven OAuth 2.0 Client Credentials standardını kullanır. Bu iki adımlı bir akıştır: önce `client_id` + `client_secret` ile kısa ömürlü bir access token alırsınız, sonra her API çağrısında bu token'ı kullanırsınız. Üstteki kod örneklerindeki istemci sınıfı bu adımı sizin için saklar; tek bir `call(...)` ile çağırırsınız. Standart tabanlı olması, herhangi bir OAuth 2.0 / OIDC kitaplığıyla çalışabilmesini sağlar.
  </Accordion>
