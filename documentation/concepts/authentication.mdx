...
description: "OAuth 2.0 üzerinden JWT Bearer token ile Payven API'lerine erişim."
---

Payven tüm public API'lerinde **OAuth 2.0 + JWT Bearer** kullanır. Token, Identity servisinden alınır; ürün API'lerine `Authorization: Bearer <token>` header'ı ile gidersiniz.

| Akış | Kullanım | Grant type |
|---|---|---|
| **Server-to-server (M2M)** | Backend entegrasyonları, otomasyonlar — bu sayfanın konusu | `client_credentials` |
| **Portal kullanıcı** | İnsan kullanıcılar (dashboard.payven.com.tr) | `password` |
| **Swagger UI / interaktif** | Local geliştirme için Swagger üzerinden interaktif login | `authorization_code` (PKCE) |

Bu sayfa **server-to-server** akışını anlatır. Portal kullanıcı login'i için: [Identity → Login](/identity/auth/login).
...
sequenceDiagram
    ...
    participant API as Ürün API (vpos / transfer / fraud)
...
JWT içinde `tenant_id`, `merchant_id`, `plan_code` claim'leri taşınır — Payven bunları okuyup işlem yetkisini doğrular. Ek bir merchant header'ı gerekmez. (Multi-merchant senaryolar için `X-Merchant-Id` ile override yapılabilir; bkz. [Sanal POS → Kimlik Doğrulama](/sanal-pos/authentication).)
...
| **SSO oturum (max)** | 10 saat | Mutlak üst sınır — refresh edilse bile 10 saat sonra yeniden token alın. |
| **Refresh token (idle)** | 30 gün | Refresh token 30 gün hareketsiz kalırsa expire olur. Düzenli kullanımda sürekli rotate olur, expire olmaz. |
...
|---|---|---|---|
| `400` | `bad_request` | İstek gövdesi geçersiz JSON veya zorunlu alan eksik | `client_id` / `client_secret` alanlarını kontrol edin |
...
| `403` | `tenant_suspended` | Kuruluş askıya alınmış | Müşteri ilişkileriniz ile iletişime geçin |
| `404` | `tenant_not_found` | `{slug}` geçersiz | Onboarding'de verilen slug'ı kontrol edin |
| `429` | `rate_limit_exceeded` | Token endpoint'ine çok istek | `Retry-After` header'ına uyun, token'ı cache'leyin |

Tam hata zarfı şekli için: [Hata Yönetimi](/documentation/concepts/errors).
...
<AccordionGroup>
  <Accordion title="Stripe gibi tek 'sk_live_xxx' secret'la çağıramaz mıyım?">
    Hayır — Payven OAuth 2.0 Client Credentials standardını kullanır. İki adımda olsa da SDK'lar bu adımı sizin için saklar; yukarıdaki örneklerde `client.call(...)` tek bir çağrıdır. Standard tabanlı olması, herhangi bir OIDC kitaplığıyla çalışabilmesini sağlar.
  </Accordion>
  ...
  <Accordion title="Birden fazla servisi tek token'la çağırabilir miyim?">
    Evet — Identity'den alınan JWT, planınızdaki tüm aktif ürünlerde geçerlidir. Sanal POS, Para Transferi, Fraud, Identity — hepsi aynı token ile çağrılır.
  </Accordion>
  <Accordion title="Test (sandbox) ortamı?">
    Sandbox endpoint'leri ayrı bir kuruluş slug'ı ile yayındadır: `https://identity-sandbox.payven.com.tr` ve `https://vpos-sandbox.payven.com.tr`. Sandbox `client_id`'leri canlıdan ayrıdır; üretim parasıyla işlem yapılmaz, kart numarası test BIN'leri kullanır.
  </Accordion>
  <Accordion title="Tüm ürünler aynı kimlik doğrulamayı mı kullanıyor?">
    Evet. Sanal POS, Para Transferi, Fraud ve Identity — hepsi `Authorization: Bearer <access_token>` bekler. API anahtarı çiftinizden token almak ürünler arası ortak adımdır; bir kez yapılır, ardından tüm ürünlere aynı token ile çağrı atılır.
  </Accordion>
</AccordionGroup>
