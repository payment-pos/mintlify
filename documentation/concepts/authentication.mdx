...
description: "Keycloak OAuth 2.0 üzerinden JWT Bearer token ile Payven API'lerine erişim."
---
...
<AccordionGroup>
  ...
  <Accordion title="API key (X-API-Key + X-API-Secret) hâlâ destekleniyor mu?">
    Evet — eski entegrasyonlar için `ApiKey` scheme aktif kalmaya devam ediyor. Yeni entegrasyonlarınız için **JWT Bearer'ı tercih edin** — daha standart, daha esnek, multi-service token tek tutuyor. API key flow'unu kullanmak zorundaysanız: bkz. [Sanal POS → Kimlik Doğrulama](/sanal-pos/authentication).
  </Accordion>
</AccordionGroup>
