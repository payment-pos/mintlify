...
Token süresi dolduğunda yenisini alır veya `refresh_token` ile yenilersin. SDK örnekleri bu döngüyü sizin için yönetir.
...
Access token içinde `tenant_id`, `merchant_id`, `plan_code` claim'leri taşınır — ürün servisleri bunları okuyup işlem yetkisini doğrular. Ek bir merchant header'ı gerekmez. (Multi-merchant senaryolar için `X-Merchant-Id` ile override yapılabilir; bkz. [Sanal POS → Kimlik Doğrulama](/sanal-pos/authentication).)
...
<AccordionGroup>
  ...
  <Accordion title="API key (X-API-Key + X-API-Secret) hâlâ destekleniyor mu?">
    Evet — eski entegrasyonlar için `ApiKey` scheme aktif kalmaya devam ediyor. Yeni entegrasyonlarınız için **OAuth 2.0 Bearer akışını tercih edin** — daha standart, daha esnek, planınızdaki tüm ürünleri tek token ile çağırır. API key flow'unu kullanmak zorundaysanız: bkz. [Sanal POS → Kimlik Doğrulama](/sanal-pos/authentication).
  </Accordion>
</AccordionGroup>
