...
| Saklama süresi | Ödeme (Sanal POS `POST /payments`): **24 saat**. Diğer yazma endpoint'leri: **3 dakika** (cache TTL). |
...
<AccordionGroup>
  ...
  <Accordion title="Aynı anahtar farklı payload ile">
    ...
    Yanıt `Content-Type: application/problem+json` (RFC 9457) formatındadır.
    Aynı anahtarla **aynı body** tekrar gönderilirse Payven cached yanıtı döner ve
    `Idempotent-Replayed: true` header'ı ekler.
  </Accordion>

  <Accordion title="TTL süresi sonrası">
    1. Saklama süresi geçmiş bir anahtar (ödemeler için 24 saat, diğer endpoint'ler için 3 dakika sonra).
