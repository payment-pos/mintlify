...
Payven, tüm API'lerinde istek limitleri uygular. Limit **kuruluşunuzun tamamına** uygulanır: aynı kuruluş altındaki tüm API anahtarları, merchant'lar ve ürünlerin (Sanal POS, Para Transferi, Identity) toplam istek sayısı kotanızı belirler. Limit, suistimali engellemek ve adil kaynak paylaşımı sağlamak içindir.
...
Genel kuruluş limitinin yanında, bazı endpoint sınıflarında **endpoint-bazlı ayrı limit** uygulanır (suistimali önleme + kaynak adaleti):
...
| **Refresh endpoint** | `POST /auth/{slug}/refresh` | **30 / dk / kuruluş** | — |
...
| **Mutabakat başlatma** | `POST /reconciliations/start` | **5 / dk / kuruluş** | EOD dosyaları nadir; yüksek hacim onboarding'de istenir |
| **Toplu transfer** | `POST /transfers/bulk/create` | **10 / dk / kuruluş** | Tek pakette 1000'e kadar transfer; paketleri birleştirin |
