...
Payven API'leri **REST + HTTP semantiği** üzerinde çalışır: HTTP durum kodu başarıyı, response body veriyi taşır. Hata gövdeleri [RFC 9457 — Problem Details](https://www.rfc-editor.org/rfc/rfc9457) standardına uyar.
...
| Kod | Anlam | Hata yanıtı MIME tipi |
|---|---|---|
...
| Header | Açıklama |
|---|---|
| `X-Correlation-Id` | Bu isteğin Payven log zincirindeki kimliği. Hata yanıtlarında body içindeki `correlation_id` alanıyla aynı değeri taşır |
