...
```json
{
  ...
  "correlationId": "9f1c8e76-2a3b-4f12-9c8d-12cb24a8a8a8"
}
```
...
| `correlationId` | İstek zinciri kimliği — destek talebinde paylaşın (yanıt header'ı `X-Correlation-Id` ile aynı değer) |

<Note>
Problem+json yanıtındaki alanlar (`type`, `title`, `correlationId` vb.) RFC 9457
ve ASP.NET ProblemDetails konvansiyonu gereği **camelCase**'tir. Diğer tüm istek
ve yanıt gövdelerinde standart **snake_case** kullanılır.
</Note>
...
| Header | Açıklama |
|---|---|
| `X-Correlation-Id` | Bu isteğin Payven log zincirindeki kimliği. Hata gövdesindeki `correlationId` ile aynı değer |
...
Tüm tarih alanları **ISO 8601** formatında ve **UTC offset**'i ile döner. Backend `DateTimeOffset` kullanır:
