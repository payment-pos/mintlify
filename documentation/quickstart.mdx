...
**Access token 5 dakika geçerli.** Production'da bunu cache'leyip auto-refresh ile yönetin — örnek istemciler: [Kimlik Doğrulama → Kod örnekleri](/documentation/concepts/authentication#sdk-ornekleri-auto-refresh).
...
| Süresi geçmiş kart | `4022770000000013` | Test bank — `provider_error_code: "54"` |
...
Bir adımda takılırsanız: yanıt header'larındaki `X-Correlation-Id` değerini (veya hata gövdesindeki `correlationId` alanını) [destek ekibine](/resources/support) iletin — log zincirini saniyeler içinde bulabiliriz.
