...
```bash
curl -X POST https://merchant-sandbox.payven.com.tr/api/v1/webhook-endpoints \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "url":            "https://your-domain.com/webhooks/payven",
    "enabled_events": ["sanalpos.payment.completed", "sanalpos.payment.failed", "sanalpos.payment.not_completed", "sanalpos.refund.completed"]
  }'
```

Webhook uçları **İşyeri API'sinde** yönetilir ve tek uç tüm ürünlerin olaylarını alır; API
anahtarınızın **İşyeri API'si (`merchant`)** kapsamı seçili olmalıdır. Yanıttaki `secret`
(`whsec_...`) yalnız bir kez döner — saklayın. Test anahtarıyla oluşan olaylar `livemode: false`
ile gelir. Ayrıntı: [Webhook'lar](/documentation/concepts/webhooks).
