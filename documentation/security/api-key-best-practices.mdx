...
description: "client_secret yönetimi, sızıntı önleme ve operasyonel güvenlik."
---

Payven API'lerine erişim **OAuth 2.0 Client Credentials** akışı ile sağlanır:
`client_id` + `client_secret` → access token. `client_secret`, kuruluşunuz adına işlem yapma yetkisidir; sızdığında saldırganın elinde sizin adınıza ödeme oluşturma kapısı olur.
