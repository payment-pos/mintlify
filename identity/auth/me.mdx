...
| Alan | Açıklama |
|---|---|
| `user` | Keycloak'tan gelen kullanıcı kimliği — `id`, `username`, `email`, `name` |
| `tenant` | Aktif tenant — `id`, `slug`, `display_name` (null olabilir, JWT'de yoksa) |
| `merchant` | Aktif merchant — multi-merchant senaryolarda override edilebilir (null olabilir) |
| `roles` | Keycloak realm rolleri (`pos-admin`, `transfer-admin`, `tenant-admin`, vb.) |
...
`/api/v1/auth/{slug}/me` endpoint'i Keycloak userinfo proxy olarak da kullanılabilir — sadece Keycloak'tan ham userinfo döner (roles + email + name), tenant/merchant/feature context'i içermez. Production akışlarında **`/api/v1/me`** tercih edin.
