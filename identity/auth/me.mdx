...
| `features[]` | Tenant planında aktif **modül** feature'ları — her birinin kullanıcının rollerinden çıkarılan effective permission listesi ve filtrelenmiş nav item'ları |
...
`/api/v1/auth/{slug}/me` endpoint'i de mevcuttur ancak yalnızca temel kullanıcı bilgisini (roller, e-posta, ad) döner — tenant / merchant / feature bağlamı içermez. Production akışlarında **`/api/v1/me`** tercih edin.
