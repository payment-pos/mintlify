...
description: "Konsoldan veya API üzerinden client kimlik bilgileri üretin."
---
...
|---|---|---|---|
| `display_name` | string | ✅ | Anahtar için açıklayıcı ad |
| `contact_email` | string | ❌ | Alarm / uyarı e-postası |
| `merchant_id` | string | ❌ | Default merchant external ID — JWT claim olarak token'a yansır |
| `plan_id` | UUID | ❌ | Atanacak plan — boş bırakılırsa kuruluş varsayılanı uygulanır |
| `daily_limit_override` | int | ❌ | Plan günlük limitini override eder (kuruş) |
| `monthly_limit_override` | int | ❌ | Plan aylık limit override |
| `rate_limit_override` | int | ❌ | Plan RPS override |
| `allowed_ips` | string | ❌ | CSV IP / CIDR listesi (örn. `52.18.42.10,10.0.0.0/8`). Boş = tüm IP'ler |
| `expires_at` | datetime | ❌ | Anahtarın otomatik pasifleşeceği tarih (UTC, ISO 8601) |
...
Aldığınız `client_id` + `secret` ile [Identity'den JWT alın](/documentation/concepts/authentication):
...
JWT içinde `tenant_id`, `merchant_id`, `plan_code`, `daily_limit`, `monthly_limit`, `rate_limit` claim'leri taşınır — ürün servisleri bu bilgileri doğrudan token'dan okur, ek bir Identity çağrısı gerekmez.
