...
description: "Server-to-server entegrasyonlar için Keycloak service-account credential'ları."
---

Payven'de "API anahtarı", **Keycloak service-account client'ıdır**. Bir API anahtarı oluşturduğunuzda Identity arka tarafta Keycloak'a yeni bir client kaydeder ve size `client_id` + `client_secret` çifti verir. Bu çift ile [`/auth/{slug}/token`](/identity/auth/login) endpoint'inden JWT alır, ürün servislerine `Authorization: Bearer ...` ile çağrı yaparsınız.
...
|---|---|---|
| `client_id` | Public tanımlayıcı, Keycloak client ID | `pvk-payven-a1b2c3` |
...
| Alan | Açıklama |
|---|---|
| `keycloak_client_id` | API çağrılarında `client_id` olarak kullanılır |
| `display_name` | İnsan-okur ad (raporlama için) |
| `merchant_id` | Bu anahtarın "default merchant"ı — JWT claim'ine yansır |
